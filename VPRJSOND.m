...
DIRECT ; TAG for use by DECODE^VPRJSON
 ...
 . . I +$P($TR(VVSTACK(VVSTACK),"e","E"),"E")=VVSTACK(VVSTACK),VVSTACK(VVSTACK) D ERRX("OBM") ; Numeric and true only
...
CURNODE() ; Return a global/local variable name based on VVSTACK
 ...
 . I +$P($TR(VVSTACK(VVI),"e","E"),"E")=VVSTACK(VVI) S VVSUBS=VVSUBS_VVSTACK(VVI) ; VEN/SMH Fix psudo array bug.
